//+------------------------------------------------------------------+
//| SybexForexAI Balance Sync EA v7                                  |
...
void OnTimer()
{
   ...
   if(StringLen(resp) > 10)
   {
      ...
      if(cnt > 0)
      {
         Print("SybexForexAI v7: ", cnt, " pending command(s) -- executing...");
         g_completedJson = ExecuteOrders(resp);
         Print("SybexForexAI v7: completedOrders=", g_completedJson);
      }
   }
...
string FetchPendingOrders()
{
   ...
   if(rc == -1)
   {
      Print("SybexForexAI v7: pull FAILED err=", GetLastError(),
            " -- add URL to Tools>Options>Expert Advisors>Allowed URLs");
      return "";
   }
   if(rc != 200)
   {
      Print("SybexForexAI v7: pull HTTP ", rc);
      return "";
   }
...
void SendDataToApp()
{
   ...
   if(rc == -1)
      Print("SybexForexAI v7: push FAILED err=", GetLastError());
   else if(rc == 200)
      Print("SybexForexAI v7: push OK | Bal=",
            DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE),2),
            " Pos=", PositionsTotal());
   else
      Print("SybexForexAI v7: push HTTP ", rc, " -- ", StringSubstr(CharArrayToString(result),0,200));
}
...
string ExecuteOrders(string response)
{
   ...
   while(true)
   {
      ...
      // Skip malformed entries with no id or symbol
      ...
      // ── modify_sl: move stop loss for an open position ──────────────
      if(isModifySl)
      {
         ...
         if(ok)
         {
            ...
            Print("SybexForexAI v7: SL modified ", symbol,
         ...
         else
         {
            ...
            Print("SybexForexAI v7: modify_sl FAILED ", symbol,
      ...
      else if(isClose)
      {
         ...
         // Prefer ticket-based close (precise); fall back to symbol-based
         if(cmdTicket > 0)
         {
            ok = CloseByTicket(cmdTicket, symbol, rc);
            if(!ok)
               Print("SybexForexAI v7: ticket close failed ticket=", cmdTicket,
         ...
         completed += "{\"id\":\"" + orderId + "\",\"success\":" + (ok ? "true" : "false")
                    + ",\"type\":\"close\"}";
         Print("SybexForexAI v7: close ", (ok ? "OK" : "FAILED"), " ", symbol);
      }

      // ── new order: place a market order ────────────────────────────
      else if(lots > 0)
      {
         ...
         if(ok)
         {
            ...
            Print("SybexForexAI v7: FILLED ", symbol, " ", direction,
         ...
         else
         {
            ...
            Print("SybexForexAI v7: REJECTED ", symbol, " retcode=", retcode);
         }
      }
      else
      {
         // Unknown command type — acknowledge to remove from queue
         completed += "{\"id\":\"" + orderId + "\",\"success\":false"
                    + ",\"error\":\"unknown command type\"}";
         Print("SybexForexAI v7: unknown command in ", StringSubstr(obj,0,80));
      }
