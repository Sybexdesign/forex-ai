...
interface JournalPageProps { trades: any[] }
...
export default function JournalPage({ trades }: JournalPageProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {/* Add Trade Modal */}
      {showAddModal && <AddTradeModal onClose={() => setShowAddModal(false)} onSave={() => setShowAddModal(false)} />}
    </div>
  )
}
...
function AddTradeModal({ onClose, onSave }: { onClose: () => void; onSave: (t: any) => void }) {
    ...
    <div style={overlayStyle} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="panel" style={{ width: 480, padding: 24, borderColor: 'var(--border-bright)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 20 }}>
          ...
          <div>
            ...
            <select value={form.pair} onChange={e => set('pair', e.target.value)} style={inputStyle}>
              {['EUR/USD', 'GBP/USD', 'USD/JPY', 'AUD/USD', 'XAU/USD'].map(p =>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button className="btn btn-primary" onClick={handleSave} style={{ flex: 2 }}>SAVE TRADE</button>
        </div>
      </div>
    </div>
  )
