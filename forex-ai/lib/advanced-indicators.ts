...
export function calcATRIndicator(candles: Candle[], pair: string): ATRResult {
  ...
  const atrPips = Math.round(atr / pipSize)

  // Volatility classification
  const volatility: 'HIGH' | 'MEDIUM' | 'LOW' =
    atrPips > 60 ? 'HIGH' : atrPips > 25 ? 'MEDIUM' : 'LOW'

  // ATR-based SL = 1.5x ATR, TP = multiples of SL
  const suggestedSL  = Math.round(atrPips * 1.5)
  const suggestedTP  = Math.round(suggestedSL * 1.5)  // 1:1.5 R
  const suggestedTP2 = Math.round(suggestedSL * 2)    // 1:2 R
  const suggestedTP3 = Math.round(suggestedSL * 3)    // 1:3 R
