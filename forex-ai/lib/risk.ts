...
import type { OpenTrade } from './oanda'
...
export function runRiskGuards(options: RiskGuardOptions): RiskCheck[] {
  ...
  // 2. Daily loss limit
  if (strategy.hardDailyStop) {
    const maxLossUSD = accountBalance * (strategy.maxLoss / 100)
    if (todayRealizedPL <= -maxLossUSD) {
      checks.push({
        allowed: false,
        severity: 'BLOCK',
  ...
  if (strategy.demoLock) {
    checks.push({
      allowed: true,
      severity: 'WARN',
      reason: 'Demo mode active — orders will be placed on OANDA practice account only.',
    })
  }
