-- mt5_webhook_sync: row-lock the config read to stop pending-order clobbering
--
-- 2026-07-03 — the push path read broker_configs.config into v_config at the
-- start of the call, then wrote pendingOrders back from that snapshot at the
-- end. An order appended via append_pending_order() between the read and the
-- write-back was silently deleted before the EA ever pulled it (observed
-- live: order queued 14:50:50Z vanished; EA never reported it). The pull
-- path's expiry-cleanup UPDATE had the same snapshot write.
--
-- Fix: SELECT ... FOR UPDATE on the initial config read. append_pending_order
-- is a plain UPDATE on the same row, so it now blocks until this function's
-- transaction commits and appends onto the freshly written config instead of
-- being overwritten. Body is otherwise identical to 20260616_mt5_rpc_mfe_mae.
