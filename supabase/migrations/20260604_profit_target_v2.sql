-- Profit Target v2: store formula components (fixedUsdTarget, profitTargetPct) alongside
-- the precomputed profitTargetUsd so the EA can verify and recompute the formula itself.
-- Formula: profitCloseAmount = profitFixedUsd × (profitTargetPct / 100)

-- ── 1. Extend set_profit_target to accept and store both components ───────────
CREATE OR REPLACE FUNCTION public.set_profit_target(
  p_token      text,
  p_value      numeric,
  p_fixed_usd  numeric DEFAULT NULL,
  p_target_pct int     DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Always write computed close amount; also persist components when provided
  v_update := jsonb_build_object('profitTargetUsd', p_value);
  IF p_fixed_usd IS NOT NULL THEN
    v_update := v_update || jsonb_build_object('profitFixedUsd', p_fixed_usd);
  END IF;
  IF p_target_pct IS NOT NULL THEN
    v_update := v_update || jsonb_build_object('profitTargetPct', p_target_pct);
  END IF;

  UPDATE broker_configs
  SET config = config || v_update
  WHERE id = v_id;

  RETURN jsonb_build_object('ok', true, 'profitTargetUsd', p_value);
END;
$$;

GRANT EXECUTE ON FUNCTION public.set_profit_target(text, numeric, numeric, int) TO anon;
GRANT EXECUTE ON FUNCTION public.set_profit_target(text, numeric, numeric, int) TO authenticated;

-- ── 2. Update mt5_webhook_sync PULL to return profitFixedUsd and profitTargetPct ─
-- Full function replace required (Postgres doesn't allow partial function edits).
...
BEGIN
  ...
  IF p_payload IS NULL OR (p_payload->>'balance') IS NULL THEN
    ...
    -- v9.2: return formula components so EA computes profitCloseAmount itself:
