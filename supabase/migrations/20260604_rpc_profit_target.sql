-- v9.1 Profit Target support:
--   1. mt5_webhook_sync PULL now returns profitTargetUsd so EA recovers after restart.
--   2. set_profit_target(token, value) — called by the app when user changes the target;
--      atomically writes to broker_configs.config without racing with EA PUSH.

-- ── 1. Atomic setter called by the app ──────────────────────────────────────
...
GRANT EXECUTE ON FUNCTION public.set_profit_target(text, numeric) TO anon;
GRANT EXECUTE ON FUNCTION public.set_profit_target(text, numeric) TO authenticated;

-- ── 2. Patch mt5_webhook_sync PULL to include profitTargetUsd ───────────────
-- Only the PULL path (p_payload IS NULL) changes — adds profitTargetUsd to response
-- so the EA automatically recovers its target after an MT5 restart without needing
-- the app to re-send a set_profit_target command.
...
BEGIN
  ...
  IF p_payload IS NULL OR (p_payload->>'balance') IS NULL THEN
    ...
    -- v9.1: include profitTargetUsd so EA recovers target after MT5 restart
