...
export function profitProtection(o) {
  ...
  const none = () => ({
    ...
    // Observable explanation of the decision. Additive field; existing consumers
    // destructure named fields and are unaffected.
    reason: null,
  ...
  /**
   * POLLING-GAP GUARD (characterisation finding #6).
   *
   * `giveback-collapse-close` exists as a BACKSTOP for a floor that was already
   * established but jumped through between two management cycles. It is not a
   * licence to close a trade that never had protection committed at all.
   *
   * Evidence that protection was previously established comes from state the
   * caller ALREADY persists — deliberately, so this needs no signature change:
   *
   *   * `retentionFloorUsd > 0`  a prior cycle computed a floor
   *   * `stage` rank > 0         a prior cycle reached a protection band
   *   * live SL above entry      a stop is ALREADY committed in profit
   *
   * Runner C is the case this fixes: 3.0R peak → 2.4R in one polling step. Every
   * earlier step was a new high, so the function returned at the `cur >= peak`
   * guard and NEVER computed a floor — meaning `stage === ''`, the floor is 0 and
   * the SL is still at/below entry. Previously that collapsed straight into a
   * market-close request at 2.4R while the trade subsequently ran to 5R.
   */
  const pipSize = Number(o.pip) > 0 ? Number(o.pip) : 0
  const slProfitUsd = (!pipSize || !Number.isFinite(Number(o.currentSl)))
    ? 0
    : ((dir === 'BUY' ? Number(o.currentSl) - o.entry : o.entry - Number(o.currentSl)) / pipSize)
      * o.pipValuePerLot * o.lots
  const priorFloorUsd = Math.max(0, Number(o.retentionFloorUsd) || 0)
  const priorStageIdx = STAGE_RANK[o.stage] ?? 0
  const protectionArmed = priorFloorUsd > 0 || slProfitUsd > 0 || priorStageIdx > 0
  ...
  if (cur < floorUsd - cfg.closeRequestMinUsd) {
    // ── POLLING-GAP RECOVERY ──────────────────────────────────────────────
    // No protection was EVER committed, so this is not a failure to protect —
    // polling simply skipped the ratchet window. Commit the achievable floor NOW
    // rather than bailing out, and never issue a close on this path.
    if (!protectionArmed) {
      // The floor may sit ABOVE current profit when the gap was jumped (Runner C:
      // floor 2.55R vs cur 2.4R). A stop above the market is not placeable, so
      // commit the best level that is still valid and in profit.
      const achievableUsd = Math.min(floorUsd, cur - cfg.minStopGapUsd)
      if (achievableUsd > 0) {
        const gapSl = dollarsToPrice(achievableUsd)
        if (improves(gapSl)) {
          return {
            ...out, newSl: gapSl, action: `ratchet-${stage.toLowerCase()}`, actionAt: nowIso,
            // Report the floor ACTUALLY COMMITTED, not the target that was
            // unreachable. `trade-manager.ts` persists max(previous, floorUsd),
            // so reporting the target here would record protection that does not
            // exist and would make the persisted floor disagree with the live SL.
            // `proposedFloorUsd` keeps the unreachable target for comparison.
            floorUsd: achievableUsd,
            reason: 'floor-committed-after-polling-gap',
          }
        }
      }
      return { ...out, reason: 'collapse-without-committed-floor' }
    }
    // Protection WAS established between cycles and price has now blown through
    // it → market-close backstop, while some profit remains (never at a loss).
    if (cur >= cfg.closeRequestMinUsd && floorPct >= 0.5) {
      return { ...out, closeRequested: true, action: 'giveback-collapse-close', actionAt: nowIso, reason: 'floor-breached-from-armed-protection' }
    }
    return { ...out, reason: 'floor-breached-below-close-threshold' }
  }
  ...
  if (gapUsd < cfg.minStopGapUsd) return { ...out, reason: 'floor-within-broker-min-gap' }
  const floorSl = dollarsToPrice(floorUsd)
  if (improves(floorSl)) {
    return { ...out, newSl: floorSl, action: `ratchet-${stage.toLowerCase()}`, actionAt: nowIso, reason: 'floor-ratchet' }
  }
  return { ...out, reason: 'current-sl-already-more-protective' }
}
