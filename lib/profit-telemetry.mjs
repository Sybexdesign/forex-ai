 ...
 * @param {object} ctx — { protectionMode, stateSeq, marketRegime, session, userId }
 */
export function toRow(item, ctx = {}) {
  const mode = ctx.protectionMode === 'live' ? 'live' : 'shadow'
  return {
    // Ownership — supplied by the SERVER from its own trusted identity (the
    // worker's WORKER_USER_ID, or the user resolved from an authenticated
    // request). Never taken from a client-supplied field: a caller must not be
    // able to attribute telemetry to another account.
...
/**
 * §7 — minimum `peakObservationCoverage` for a lifecycle to count as faithfully
 * observed. Purely a DIAGNOSTIC flag: it never changes a P&L figure, it only
 * marks whether the observation cadence captured enough of the real trajectory
 * for a counterfactual conclusion to be drawn from this lifecycle.
 */
export const PEAK_COVERAGE_MIN = 0.7
...
export function closeSummaryFromRows(rows, over = {}) {
  ...
  // §7 — MFE RECONCILIATION (diagnostic only, never a substitution).
  //
  // `peakProfit` is what the SHADOW actually observed; `mfe` is the broker's own
  // realised peak from `trades.mfe_usd`. Their ratio estimates how much of the
  // real trajectory the observation cadence managed to capture. A low value says
  // this lifecycle's shadow peak is an UNDERSTATEMENT, so any counterfactual
  // conclusion drawn from it is unreliable.
  //
  // The shadow peak is deliberately NOT replaced by the broker figure — the two
  // are reported side by side so coverage stays visible rather than being
  // silently repaired into a better-looking number.
  const brokerMfe = over.actualMfeUsd != null ? Number(over.actualMfeUsd) : null
  const peakObservationCoverage = brokerMfe != null && brokerMfe > 0
    ? Math.round((Math.max(0, peakProfit) / brokerMfe) * 10000) / 10000
    : null
  const peakObservationReliable = peakObservationCoverage == null
    ? null
    : peakObservationCoverage >= PEAK_COVERAGE_MIN
  return {
    ...
    riskUsd: risk,
    peakR,
    // §7 — observation-fidelity diagnostics. Reported alongside the figures, never
    // fed into them: `shadowObservedPeakUsd` is what the cadence actually saw and
    // `brokerMfeUsd` is ground truth, kept separate so a coverage gap is visible.
    shadowObservedPeakUsd: Math.max(0, peakProfit),
    brokerMfeUsd: brokerMfe,
    peakObservationCoverage,
    peakObservationReliable,
  }
}
